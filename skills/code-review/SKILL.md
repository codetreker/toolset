...
8. **分支基准** — PR 的 base branch 是否包含所有已合并的 hotfix？如果分支是从旧代码开的，squash merge 可能丢失中间合入的修复（尤其是安全修复）。合并前必须 `git log main --oneline` 确认无遗漏。
...
用 coding agent（Claude Code 或 Codex）执行 review。调用参数和超时见 `using-coding-agent` skill。
...
4. 用模板组装 prompt
5. 用 coding agent 跑 review（调用方式见 `using-coding-agent` skill）
