...
派 subagent 干活的标准流程：**调研 → Review → 执行**。
...
2. Spawn subagent（coding agent），任务描述必须包含：
   - 明确说明**只调研不执行**
   - 调研完把结果写到 `~/.openclaw/subagents/_tasks/{task-id}.md`
   - 文件格式见下方模板
3. 等 subagent 完成

**调研 subagent 铁律**：不做任何写操作（不改现有文件、不执行变更命令、不发消息）。唯一允许的写操作是创建 task 文件本身。
...
**同时派 2 个 subagent 做独立 review**，尽量用不同的 coding agent（如 Claude Code + GitHub Copilot / Codex），避免单一模型盲区。

#### 流程

1. **同时 spawn 2 个 review subagent**
   - 每个 subagent 读 `~/.openclaw/subagents/_tasks/{task-id}.md`
...
1. Spawn 新的 subagent（coding agent），任务描述必须包含：
...
**执行 subagent 铁律**：严格按方案走，发现偏差先停下来报告，不要自作主张。

## 铁律总结

1. 不确定的事先调研，确认了再执行
2. 调研 subagent 只读不写（除 task 文件本身）
3. 执行 subagent 严格按方案走，偏差即停
